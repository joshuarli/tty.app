...
- **The tiled renderers are experimental.** Launching with `TTY_TILED_RENDER=1` selects the guarded full cell-tiled compute path. `TTY_TILED_DAMAGE=1` selects the guarded retained active-cell path, which keeps a persistent GPU surface and copies it into each drawable. The default remains the full-frame compute shader; both switches exist for onscreen comparison and are not production defaults.
