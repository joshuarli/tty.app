...
- **The tiled renderer is experimental.** Launching with `TTY_TILED_RENDER=1` selects a guarded cell-tiled compute path in `MetalRenderer`; the default remains the full-frame compute shader. The switch exists for onscreen comparison and is not a production default.
